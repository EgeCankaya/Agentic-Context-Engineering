...
from verify_setup import (
    check_gpu as verify_check_gpu,
)
from verify_setup import (
    check_ollama as verify_check_ollama,
)
from verify_setup import (
    test_generation as verify_test_generation,
)
...
@click.group()
@click.option(
    "--config", "-c", default="agentic_context_engineering/configs/default.yaml", help="Path to configuration file"
)
...
@click.option("--playbook", "-p", help="Path to initial playbook (defaults to latest if available)")
@click.option("--output", "-o", help="Output directory")
@click.option("--eval-dataset", "-e", help="Path to evaluation dataset (used for gating BLEU/EM)")
@click.pass_context
def run(ctx, iterations, tasks, playbook, output, eval_dataset):
    """Run ACE iterations"""
    try:
        # Initialize runner
        ...
        # Optional evaluation dataset for gating
        ...
        # Run iterations
        print("Running ACE iterations...")

        results = runner.run_iterations(
            num_iterations=iterations, tasks=task_list, playbook_path=playbook, evaluation_dataset=evaluation_dataset
        )
...
@cli.command()
@click.option("--results-dir", "-r", help="Directory with metrics_history_*.json (defaults to configured output dir)")
@click.pass_context
def summarize(ctx, results_dir):
    """Summarize metrics trends across iterations."""
    try:
        runner = ACERunner(config_path=ctx.obj["config_path"])
        base_dir = Path(results_dir) if results_dir else runner.output_dir
        files = list(base_dir.glob("metrics_history_*.json"))
        if not files:
            console.print("[red]No metrics history files found[/red]")
            return
        latest = max(files, key=lambda f: f.stat().st_mtime)
        with open(latest) as f:
            hist = json.load(f)

        table = Table(title=f"Metrics Summary ({latest.name})")
        table.add_column("Iter", justify="right")
        table.add_column("BLEU")
        table.add_column("EM")
        table.add_column("ROUGE-L")
        table.add_column("SemSim")
        table.add_column("ΔBLEU")
        table.add_column("ΔEM")
        for i, b, em, rl, ss, db, de in zip(
            hist.get("iteration", []),
            hist.get("bleu", []),
            hist.get("exact_match", []),
            hist.get("rougeL", []),
            hist.get("semantic_similarity", []),
            hist.get("delta_bleu", []),
            hist.get("delta_exact_match", []),
        ):
            table.add_row(str(i), f"{b:.3f}", f"{em:.3f}", f"{rl:.3f}", f"{ss:.3f}", f"{db:+.3f}", f"{de:+.3f}")
        console.print(table)
...
@cli.command()
@click.option("--dev-iterations", "-i", type=int, default=3, help="Number of dev iterations before holdout eval")
@click.pass_context
def validate(ctx, dev_iterations):
    """Run short dev cycle and holdout evaluation to verify gates."""
    try:
        runner = ACERunner(config_path=ctx.obj["config_path"])
        with Progress(
            SpinnerColumn(), TextColumn("[progress.description]{task.description}"), console=console
        ...
        # Print brief summary
        dev = results.get("dev_iterations", {})
        final_pb = dev.get("final_playbook")
        if final_pb:
            console.print(f"Final playbook: v{final_pb.version}, status: {final_pb.metadata.convergence_status}")
        holdout = results.get("holdout_evaluation", {})
        qm = holdout.get("quantitative_metrics", {})
    ...
    if results["iterations"]:
        ...
        for i, iteration in enumerate(results["iterations"], 1):
            if iteration["success"]:
                ...
                console.print(f"Iteration {i}: OK (Score: {metrics.get('avg_reflection_score', 0.0):.3f})")
            else:
                console.print(f"Iteration {i}: FAIL ({iteration.get('error', 'Unknown error')})")
